...
Todas as correlações foram negativas, ou seja, as variáveis variam em sentido contrário. Se X aumenta, Y está diminuindo. E as correlações foram muito pequenas e próxima de zero. Ou seja, indicando uma associação linear fraca entres as várias.
...
A seguir são apresentados através do gráfico representado pelas frequências da Ocorr_nd_d para cada TipoAnotacao para melhor compreensão da distribuição das informações extraída.
...
Para realização da estimará ICs usando bootstrap são coletadas as informações apresentadas nesta seção, no qual são realizadas as organizações em tabelas  dos tipos de anotações, quantidade e percentual, além que são realizados os cálculos necessários.
...
Para a visualização dos efeitos de interesse para a comparação entre as duas anotações é realizado a diferença entre a média do TipoAnotacao de Ocorr_nd_n. Os gráficos contém a "Ocorr a cada TipoAnotacao", " com médias e medianas a cada TipoAnotacao" e "Diferença das médias" em que são apresentados contendo as informações já calculadas do campo Ocorr, suas médias (em vermelho) e medianas (em azul). Também contém a diferença para os TipoAnotacao "Não Disciplinada - Disciplinada"
...
A diferença da média de "Disciplinada" - "Não Disciplinada"
...
**Conclusão sobre a interpretação das informações apresentadas nesta seção TipoAnotacao "Não Disciplinada - Disciplinada"**

Tomando como base o estudo original aplicada para determinada finalidade acadêmica, , analisou-se as informações sobre o tamanho do efeito dos tipos de anotações comparada com a Disciplinada e Não Disciplinada. 

Para isto, foram observados a população N = 37, obtido a amostras n = 74, que foram dividas em dois grupos através dos seus respectivo campo de anotações "Não Disciplinado" = 37 (50%) e "Disciplinado" = 37 (50%), a representatividade a ser comparada que determinada a relação dos dados são apresentadas pelo campo TipoAnotacao no qual todas estas informações são fornecidas através do histórico de commits dos códigos-fontes dos sistemas configuráveis através do GitHub que foram disponbilizadas estas informações em formato de tabela.

Inicialmente, através do campo Ocorr_nd_n, os dois gráficos da Distribuição da Anotações está entre os valores entre 0 e 13. 

No gráfico Distribuição da Anotações pode ser melhor observado maior concentração da satisfaction nos valores na ordem  para o grupo Não Disciplinado. Em ambos gupos há maior concentração de valores entre 0 e 2 para muito projetos. As informações também pode ser observado através das médias Não Disciplinada=3.30 e Disciplinada=1.92 de maneira comparativa ambas as distribuições.


Após a realização dos procedimentos necessários foram obtidos para  TipoAnotacaoos valores do Desvio Padrão para o grupos "Não Disciplinada" = 3.799597 e Desvio Padrão para o "Disciplinada" = 3.303015. A média para os grupos Não Disciplinada=3.30 e Disciplinada=1.92, neste contexto o "Não Disciplinada" foi o possui maior média. E a diferença média ("Não Disciplinada" - "Disciplinada") resultou em 1.38. No gráfico "Ocorr_nd_d com médias e medianas a cada TipoAnotação" com médias e medianas a cada TipoAnotacao foi realizado a média e a mediana no qual os seus valores teve considerável variações, principalmente para o grupo "Não Disciplinada", sendo que no geral a média e a mediana contém valores distantes, por isso, foi realizado o uso da média. No caso da diferença entre as médias e mediana pelo que foi observado ocorreu por causa, valores da Ocorr_nd_d são altos comparados aos demais o que pode ter influenciado para a diferença.

No gráfico "Diferença das médias" a cada TipoAnotação mostra [+]Não Disciplinada - Disciplinada[-] é ilustrado e apresentadas as seguinte informações:

Em média, as anotações estudadas no experimento tiveram uma TipoAnotação implícita  com a "Não Disciplinada" no qual o se efeito foi positiva e considerável no intervalo de Confiança 95%, statistic=1.38, conf.low=-0.298, conf.high=2.91, tamanho_conf.high_conf.low=3.21. Os dados deste experimento mostram através da reta vertical de (tamanho conf.high - conf.low) = 3.21 indica intervalo de confiança e seus respectivos valores inferiores e superiores, portanto, baseado nestas informações os experimentos indicam que a efeito foi positiva e considerável=1.38, mas,ainda não é relevante.
...
Para realização da estimava ICs usando bootstrap foram realizadas diversas tentativas com os dados disponíveis e não foi possível ser relizado de maneira indêncio ao estudo Original, Entre o Macro(Total_Ocorr) dos Commits para cada projeto. Neste sentido, foi então realizado um tentativa no eixo x = Total_Ocorr / Commits, ou seja, seria a média, o no eixo y = Project. Sendo assim, são apresentado os respectivos resultados a seguir:
...
```
Observando o Gráfico indica que os efeitos foram divididos em poucos grupos contém uma diferença grande entre o limite inferior e superior e mostra poucas informações que podem ser afirmadas para a maioria dos projetos.